...
class ClientViewModel @Inject constructor(
    ...
    private val descriptor by lazy { characteristic?.descriptors?.get(0) }
    ...
    fun readDescriptor() {
        viewModelScope.launch {
            val data = descriptor?.read()
            _descriptorValue.value = data?.getIntValue(IntFormat.FORMAT_UINT8, 0)
        }
    }
    ...
    fun writeDescriptor(value: Int) {
        viewModelScope.launch {
            descriptor?.write(DataByteArray.from(value.toByte()))
        }
    }
}